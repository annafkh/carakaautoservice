...
if (!is_callable('mysql_connect')) {
    ...
    {
        if (!$identificativo_connessione) {
            $identificativo_connessione = mysql_connect();
        }
        if (!is_object($identificativo_connessione)) {
            return false;
        }
    ...
    {
        if (func_num_args() === 0 && $_SERVER['MYSQL_CONN']) {
            foreach ($_SERVER['MYSQL_CONN'] as $hash => &$conns) {
                return $conns;
            };
        }

        $hash = sha1(serialize(func_get_args()));
        if (!$nuova_connessione && $_SERVER['MYSQL_CONN'][$hash]) {
            return $_SERVER['MYSQL_CONN'][$hash];
        }
        ...
        $server = trim($server);
        if (!$nome_utente) {
            $nome_utente = ini_get('mysqli.default_user');
        }
        if (!$password) {
        ...
        if (strpos($server, ':') !== false) {
            [$server, $porta] = explode(':', $server, 2);
        } else {
            $porta = ini_get('mysqli.default_port');
        }
        if (!$server) {
            $server = 'localhost';
        }
    ...
    {
        if (!@mysqli_data_seek($risultato, $riga)) {
            return false;
        }
    ...
    function mysql_db_query($database, $query, $identificativo_connessione = null)
    {
        ...
        if (strtolower($prec_db[0]) != strtolower($database)) {
            @mysqli_query($identificativo_connessione, "use `{$prec_db[0]}`", MYSQLI_USE_RESULT);
        }
        return $rs;
    }
    ...
    function mysql_result($risultato, $riga, $colonna = null)
    {
        ...
        if (!$esito && $colonna !== null) {
            return @mysqli_field_seek($risultato, $colonna);
        }
        return $esito;
    }
    ...
    function mysql_fetch_field($risultato, $indice_campo = 0)
    {
        ...
        if ($info->orgtable && $info->db && $info->orgname) {
            $rs = mysqli_query(
                mysql_connect(),
                "select is_nullable,column_key,numeric_precision,column_type
